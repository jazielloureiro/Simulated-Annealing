...
if __name__ == '__main__':
    ...
    for i in range(instance.itens_amount):
        if instance.solution[i] == 1:
            itens_added.append(f'Value: {instance.itens[i].value}\tWeight: {instance.itens[i].weight}')
            total_weight += instance.itens[i].weight
        else:
            itens_not_added.append(f'Value: {instance.itens[i].value}\tWeight: {instance.itens[i].weight}')
